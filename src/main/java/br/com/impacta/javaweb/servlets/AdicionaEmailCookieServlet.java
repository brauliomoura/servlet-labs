...
public class AdicionaEmailCookieServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//recuperar o parametro email enviado no formulário e adicionar o valor como cookie no browser do usuário
		String emailInformado = request.getParameter("email");
		Cookie cookieEmail = new Cookie("email",emailInformado);
		response.addCookie(cookieEmail);
		
		response.sendRedirect("http://localhost:8080/servlet-labs/login.html");
